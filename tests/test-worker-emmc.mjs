...
void setup() {
  ...
  Serial.printf("MMC_INIT=%d\\n", (int)err);
  if (err != 0x106) { Serial.println("MMC_INIT=FAIL"); pass = false; }
  else Serial.println("MMC_INIT=PASS");
  Serial.printf("IS_MMC=%d RCA=%u CAP=%u\\n", (int)card.is_mmc, (unsigned)card.rca, (unsigned)card.csd.capacity);
  ...
  Serial.printf("EXTCSD rev=%u type=%u seccount=%u\\n", (unsigned)ext[192], (unsigned)ext[196], (unsigned)sec_count);
...
async function run() {
  ...
  await proxy.init('ESP32', { flashSizeMB: 4, mmuPages: 64, strapValue: 0x13, budget: 5000000, sdCard: { sizeMB: 16, type: 'mmc' } }, flash, rom);
  proxy.run();
  ...
  console.log(out);
  if (!out.includes('RESULT=PASS') || !out.includes('MMC_INIT=PASS') || !out.includes('MMC_NEG=PASS') || !out.includes('MMC_EXTCSD=PASS') || !out.includes('MMC_DATA=PASS') || !out.includes('ALL TESTS PASSED')) {
