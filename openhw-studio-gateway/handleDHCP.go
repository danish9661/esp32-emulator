...
// uport is the host-side listen port for THIS board (allocated per-IP like
// the TCP forward above, NOT a fixed 5683: a second board's bind would
// otherwise fail and steal the first board's forward). Reported to the
// host via the UDP_FORWARD message so tests send to the right port.
	...
	if gatewayMode == "public" {
		...
		if !hasProxy {
			...
			for {
				...
				// API is TCP-only) — see handleUDPProxy. Per-board port
				// (never blindly 5683): with a shared gateway, board N+1's
				// DHCP would otherwise fail to bind 5683 (held by board 1)
				// and steal board 1's forward — host reqs then inject to
				// the WRONG board while the test polls its own (COAP_RX=0
				// HOST_ACKS=0 with a healthy boot + association). Reuse the
				// existing listener when this IP already has one.
				uport := 5683
				udpFwdMu.Lock()
				if l, ok := udpFwdListeners[ipStr]; ok {
					uport = l.uport
